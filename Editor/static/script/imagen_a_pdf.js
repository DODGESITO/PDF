class ImageToPDFConverter {
  ...
  bindEvents() {
    ...
    this.marginsSelect.addEventListener("change", (e) =>
      this.handleMarginsChange(e)
    );
    ...
    if (this.moveUpBtn)
      this.moveUpBtn.addEventListener("click", () => this.moveImage(-1));
    if (this.moveDownBtn)
      this.moveDownBtn.addEventListener("click", () => this.moveImage(1));

    if (this.removeCurrentBtn) {
      this.removeCurrentBtn.addEventListener("click", () =>
  ...
  async handleFileSelect(event) {
    ...
    for (const file of files) {
      ...
      if (file.size > maxSize) {
        this.showMessage(
          `${file.name} es demasiado grande. Máximo 10MB por imagen`,
          "warning"
        );
        continue;
      }

      // Verificar si la imagen no está corrupta
      const isValid = await this.verifyImageIntegrity(file);
      if (!isValid) {
        this.showMessage(
          `${file.name} está corrupta o no se pudo leer`,
          "error"
        );
        continue;
      }
    ...
    this.showMessage(
      `${validFiles.length} imagen(es) agregada(s). Total: ${this.imageFiles.length}`,
      "success"
    );
  }

  async verifyImageIntegrity(file) {
  ...
  drawImageOnCanvas(ctx, img, canvasWidth, canvasHeight, imageFit, margins) {
    ...
    switch (imageFit) {
      case "fit":
        ...
        //     drawX = margins.left + (availableWidth - imgWidth) / 2
        //     drawY = margins.top + (availableHeight - imgHeight) / 2
        //     break
    }
  ...
  moveImage(direction) {
    ...
    this.showMessage(
      `Imagen movida. Nueva posición: ${this.currentIndex + 1}`,
      "info"
    );
  }
  ...
  removeImageByName(fileName) {
    ...
    if (index !== -1) {
      ...
      this.showMessage(
        `Imagen eliminada. Total: ${this.imageFiles.length}`,
        "info"
      );
    }
  }
  ...
  updateNavigationUI() {
    if (this.imageFiles.length > 1) {
      ...
      this.pageNumber.textContent = `${this.currentIndex + 1} / ${
